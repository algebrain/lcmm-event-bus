...
Используйте `:db/type :filelog`.

Минимальная конфигурация:

```clojure
{:db/type :filelog
 :filelog/config {:path "./data/event-bus.log"}}
```

Поля `:filelog/config`:

- `:path` — путь к WAL‑логу (обязателен).
- `:sync?` — выполнять `fsync` на каждую транзакцию (`true` по умолчанию).
- `:fsync-interval-ms` — батч‑fsync: транзакции подтверждаются после группового `fsync` (без потерь, но выше latency).
  Значение имеет приоритет над `:sync?`.
- `:queue-size` — размер очереди обработчиков (по умолчанию `8192`).

Особенности:

- WAL append‑only, состояние обработчиков хранится в памяти.
- После рестарта состояние восстанавливается из WAL.
- ВАЖНО: статусы обработчиков на диск не пишутся. После рестарта **незавершённые транзакции не продолжают обработку автоматически**,
  потому что идентификаторы обработчиков относятся к текущему процессу. Такие транзакции будут помечены как `:failed` с ошибкой
  `:handler-missing`, если обработчики из прошлого процесса не найдены.
  Обработчики всё равно должны быть идемпотентными на случай повторной обработки в рамках одного процесса.
- Режим `:fsync-interval-ms` увеличивает throughput ценой latency, но не допускает потерь транзакций.
- `:sync? false` ускоряет запись, но допускает потерю последних транзакций при падении процесса.
...
- `:tx-retention-ms` — срок хранения успешных транзакций (мс, по умолчанию `7 дней`).
- `:tx-cleanup-interval-ms` — периодичность фоновой очистки (мс, по умолчанию `1 час`).

### Очистка успешных транзакций

`transact` хранит рабочие данные во внутренней БД. Чтобы база не росла бесконечно, есть фоновая очистка:

- Удаляются только транзакции со статусом `:ok`, которые старше `:tx-retention-ms`.
- `:failed` и `:pending` не удаляются.
- Очистка запускается периодически с интервалом `:tx-cleanup-interval-ms`.
