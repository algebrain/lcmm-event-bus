...
## 4. Доставка и Консистентность (Durability & Consistency)

- Шина **не** обеспечивает атомарность между коммитом бизнес-данных и публикацией события.
- Долговременный режим гарантирует доставку **как минимум один раз** после записи события в хранилище.
- Обработчики должны быть идемпотентными.
- Для атомарности требуется outbox в БД модуля-отправителя или аналогичный механизм на уровне приложения.
