...
- `:durable`: Строка-путь включает долговременный режим и задает каталог хранения (Datahike `:file` по умолчанию).
- `:persistence`: Пользовательская реализация хранилища (реализует `event-bus.persistence/PersistenceStore`).
- `:dispatcher-enabled`: Включить/выключить фонового исполнителя (по умолчанию `true`).
- `:dispatcher-interval-ms`: Интервал опроса хранилища (по умолчанию `500`).
- `:dispatcher-batch-size`: Размер пакета сообщений за цикл (по умолчанию `100`).
- `:dispatcher-max-attempts`: Лимит попыток доставки (по умолчанию `5`).
...
- **Сигнатура:** `(subscribe bus event-type handler & {:keys [schema meta inbox]})`
...
- **`:inbox`:** (опционально) Включает дедупликацию по `message-id` через хранилище. Требует `:durable` или `:persistence`.
...
Если включен `:durable`, `publish` сохраняет событие в хранилище и возвращает `envelope`; фактическая доставка выполняется фоновым исполнителем.
