...
- Буфер считает ожидающие события, а не `(событие × подписчик)`.
- `publish` в этом режиме остается fail-fast:
  - если в очереди нет места, исключение прилетает сразу;
  - вызывающий поток не ждет освобождения очереди.
- Fanout по подписчикам происходит после dequeue, поэтому число listeners не съедает буфер заранее.
...
Практический смысл такого контракта:

- критический путь LCMM не блокируется из-за фоновых обработчиков;
- `buffer-size` читается буквально как емкость очереди событий;
- уменьшается память, удерживаемая очередью при fanout-heavy нагрузке.
